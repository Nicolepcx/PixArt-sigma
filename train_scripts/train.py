...
def train():
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % config.log_interval == 0 or (step + 1) == 1:
                ...
                info = f"Step/Epoch [{(epoch-1)*len(train_dataloader)+step+1}/{epoch}][{step + 1}/{len(train_dataloader)}]:total_eta: {eta}, " \
                       f"epoch_eta:{eta_epoch}, time_all:{t:.3f}, time_data:{t_d:.3f}, lr:{lr:.3e}, s:({unwrapped_model.h}, {unwrapped_model.w}), "
            ...
            synchronize()
            if accelerator.is_main_process and ((epoch - 1) * len(train_dataloader) + step + 1) % config.save_model_steps == 0:
                os.umask(0o000)
        ...
        synchronize()
        if accelerator.is_main_process and (epoch % config.save_model_epochs == 0 or epoch == config.num_epochs):
            final_checkpoint_path = os.path.join(config.work_dir, 'checkpoints', f'epoch_{epoch}_step_{step+1}.pth')
            os.umask(0o000)
            save_checkpoint(final_checkpoint_path,
                            epoch=epoch,
                            step=(epoch - 1) * len(train_dataloader) + step + 1,
                            model=accelerator.unwrap_model(model),
                            model_ema=accelerator.unwrap_model(model_ema),
    ...
    # After all epochs are completed, upload the final checkpoint
    if accelerator.is_main_process:
        api = HfApi()
        api.upload_file(
            path_or_fileobj=final_checkpoint_path,
            path_in_repo=f"checkpoints/epoch_{epoch}_step_{step+1}.pth",
            repo_id=args.hf_repo_id,
            use_auth_token=args.hf_token
        )
        logger.info(f"Final checkpoint uploaded to Hugging Face Hub: {final_checkpoint_path}")
...
def parse_args():
    ...
    # Add arguments for Hugging Face Hub credentials
    parser.add_argument("--hf_token", type=str, help="Hugging Face token for authentication")
    parser.add_argument("--hf_repo_id", type=str, default=None, help="Repository ID for Hugging Face Hub")
